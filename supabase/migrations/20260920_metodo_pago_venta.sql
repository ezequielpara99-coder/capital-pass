-- Capital Pass: registrar como cobro cada venta en persona (RRPP y puerta):
-- efectivo o transferencia, para que el organizador tenga el detalle
-- completo de como entro cada peso.
begin;

create type public.sale_payment_method as enum ('efectivo', 'transferencia');

alter table public.sales add column if not exists payment_method public.sale_payment_method;

-- Postgres identifica una funcion por nombre + tipos de parametros: agregar
-- un parametro (aunque tenga default) no reemplaza la version anterior con
-- CREATE OR REPLACE, crea una sobrecarga ambigua al lado. Hay que borrar la
-- version vieja de 8 parametros primero.
drop function if exists public.create_sale(uuid, uuid, integer, text, text, text, text, text);

-- create_sale: agrega p_payment_method (obligatorio para RRPP y puerta,
-- no aplica al organizador vendiendo directo).
create or replace function public.create_sale(p_event_id uuid, p_ticket_type_id uuid, p_quantity integer, p_buyer_first_name text, p_buyer_last_name text, p_buyer_dni text, p_buyer_phone text, p_buyer_email text DEFAULT NULL::text, p_payment_method text DEFAULT NULL::text)
