...
export async function GET() {
  try {
    ...
    const { data: membership } = await admin
      ...
      // usuario es organizador activo de mas de una organizacion a la vez.
    ...
    const memberIds = staffRows.map(
      (staff) => staff.organization_member_id
    );
    ...
    for (const ticket of tickets) {
      ...
      ticketsBySale.set(
        ticket.sale_id,
        (ticketsBySale.get(ticket.sale_id) ?? 0) + 1
      );
    }
    ...
    for (const sale of sales) {
      if (!sale.seller_member_id) {
        continue;
      }

      const current =
        performanceByMember.get(sale.seller_member_id) ?? {
          salesCount: 0,
          ticketsSold: 0,
          totalSold: 0,
        };
    ...
    const rrpps = staffRows
      .map((staff) => {
        ...
        const performance =
          performanceByMember.get(member.id) ?? {
            salesCount: 0,
            ticketsSold: 0,
            totalSold: 0,
          };
    ...
    const mappedRRPPs = rrpps.filter((rrpp) => rrpp.mapped).length;
    ...
    const totalSold = rrpps.reduce(
      (total, rrpp) => total + rrpp.totalSold,
      0
    );

    const ticketsSold = rrpps.reduce(
      (total, rrpp) => total + rrpp.ticketsSold,
      0
    );
    ...
    return NextResponse.json({
      ...
      metrics: {
        ...
        activeRRPPs,
        mappedRRPPs,
        unmappedRRPPs: rrpps.length - mappedRRPPs,
    ...
    console.error("GET /api/rrpps/mapa", error);
