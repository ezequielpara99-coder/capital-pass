-- Bug real encontrado en revision: create_sale (venta en puerta y RRPP)
-- no tenia ninguna forma de detectar un reintento del mismo pedido, a
-- diferencia de create_bartender_sale/redeem_combo_ticket que ya se
-- arreglaron en 20260951. Si el vendedor de puerta cobra en efectivo,
-- aprieta "Confirmar venta", el servidor la registra (descuenta cupo,
-- crea comprador/venta/entradas) pero la respuesta se pierde por un
-- corte de wifi (tipico en la puerta de un evento), el formulario queda
-- habilitado para reintentar con los mismos datos: se genera una
-- SEGUNDA venta completa, doble cobro potencial y cupo descontado dos
-- veces. Mismo patron de fix: una idempotency key opcional que el
-- cliente genera una sola vez por intento y reenvia igual en cada
-- reintento.
begin;
...
begin
  ...
  insert into public.sales (organization_id, event_id, buyer_id, seller_member_id, status, total_minor, currency, channel, payment_method, confirmed_at, idempotency_key)
  values (v_organization_id, p_event_id, v_buyer_id, v_seller_member_id, 'confirmed', v_total, v_ticket_currency, v_sale_channel, v_payment_method, now(), p_idempotency_key)
